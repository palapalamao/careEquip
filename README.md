...
- 设备详情、点位读数、数据质量；本地模拟模式提供固定的 24 小时趋势。`n- 运维工单（F-A）：创建、open→inProgress→closed 单向状态迁移、留痕导出；对应标准 9.1.1/9.2.9。`n- 定期检测记录（F-A）：饮用水/医疗用水/医用气体/空调/污水/射线防护等检测对象登记与留痕；对应标准 9.1.2。`n- 设备档案（F-A）：验收日期/投用日期/验收资料，仅允许补充空缺字段；设备详情「档案」「运维记录」时间线。
